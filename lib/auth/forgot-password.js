...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!isEmail) {
      // It's a phone number. Ensure a record exists in the pending collection
      const usersCollection = await getCollection('users');
      const pendingCollection = await getCollection('pending');
      
      if (usersCollection && pendingCollection) {
        let userRow = await usersCollection.findOne({ Phone: email });
        if (userRow) {
          ...
          const pendingUser = { ...userRow, isVerified: false };
          delete pendingUser.Address;
          delete pendingUser.Measurements;
          await pendingCollection.insertOne(pendingUser);
          await usersCollection.deleteOne({ _id: userRow._id });
        } else {
          // If not in users, check if they exist in pending
          userRow = await pendingCollection.findOne({ Phone: email });
          if (!userRow) {
            // Create a pending placeholder user so that the admin can manually register/verify them
            const newPending = {
              ID: Date.now().toString(),
              CreatedAt: new Date().toISOString(),
              Name: "Pending User (WhatsApp Reset)",
              Email: '',
              Phone: email,
              PasswordHash: '',
              isVerified: false
            };
            await pendingCollection.insertOne(newPending);
          }
        }
      }
